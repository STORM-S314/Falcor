...
def render_graph_Optix():
    ...
    g.create_pass('OptixPass', 'OptixDenoiser', {})
    
    g.add_edge('PathTracer.color', 'OptixPass.color')
    g.add_edge('PathTracer.albedo', 'OptixPass.albedo')
    g.add_edge('PathTracer.guideNormal', 'OptixPass.normal')
    g.add_edge('GBufferRT.mvec', 'OptixPass.mvec')
    ...
    g.add_edge('OptixPass.output', 'TAA.colorIn')
...
try: 
    ...
    # frame capture
    m.frameCapture.outputDir = "/home/lijing/data/Denoised/Optix"
    ...
    with open(f"{m.frameCapture.outputDir}/Optix.csv", "w") as f:
