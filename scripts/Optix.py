...
optix_path = denoised_frame_path + '/Optix'
...
try: 
    ...
    # frame capture
    optix_path += '/' + scene_path.split('/')[-1].split('.')[0] + f'/{m.clock.framerate}FPS'
    if not os.path.exists(optix_path):
        os.makedirs(optix_path)
    m.frameCapture.outputDir = optix_path
    ...
    with open(optix_path + "/Optix.csv", "w") as f:
