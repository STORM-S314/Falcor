...
def render_graph_ASVGF():
    ...
    g.create_pass('ASVGFPass', 'ASVGFPass', {'UseCSVGF': useCSVGF, 'NumIterations': 5, 'HistoryTap': 0, 'FilterKernel': 1, 'TemporalColorAlpha': 0.05000000074505806, 'TemporalMomentsAlpha': 0.10000000149011612, 'DiffAtrousIterations': 5, 'GradientFilterRadius': 2})
...
try: 
    ...
    for i in range(frames + 1):
        ...
        #     print(f"\rProgress: {i + 1}/{frames} frames captured")
    capture = m.profiler.end_capture()
    m.profiler.enabled = False

    frameCount = capture["frame_count"]
    lastFrameTime = capture["events"]["/onFrameRender/RenderGraphExe::execute()/ASVGFPass/gpu_time"]["records"][frameCount - 1]
    meanFrameTime = capture["events"]["/onFrameRender/RenderGraphExe::execute()/ASVGFPass/gpu_time"]["stats"]["mean"]
    print(f"Frame Count: {frameCount}")
    print(f"Last frame gpu time: {lastFrameTime} ms")
    print(f"Mean frame gpu time: {meanFrameTime} ms")
    exit()
    # # camera = m.scene.camera
    ...
    print(e)
#C:\Users\storm\Documents\GitHub\Falcor\build\windows-vs2022\bin\Release\Mogwai.exe --headless --script="C:\Users\storm\Documents\GitHub\Falcor\scripts\ASVGF.py" -v2 --gpu=0
