...
try: 
    ...
    # frame capture
    svgf_path += '/' + scene_path.split('/')[-1].split('.')[0] + f'/{m.clock.framerate}FPS'
    if not os.path.exists(svgf_path):
        os.makedirs(svgf_path)
    ...
    with open(svgf_path + "/SVGF.csv", "w") as f:
        ...
        for i in range(frameCount):
            if i >=start_frame_idx and i%step == 0:
                meanFrameTime_denoise += capture['events']['/onFrameRender/RenderGraphExe::execute()/SVGFPass/gpu_time']['records'][i] + capture['events']['/onFrameRender/RenderGraphExe::execute()/SVGFPass0/gpu_time']['records'][i]
