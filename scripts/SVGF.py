...
svgf_path = denoised_frame_path + '/SVGF'
...
def render_graph_SVGF():
    ...
    g.add_edge('GBufferRT.pnFwidth', 'SVGFPass.PositionNormalFwidth')
    ...
    g.add_edge('GBufferRT.emissive', 'SVGFPass0.Emission')
    g.add_edge('GBufferRT.posW', 'SVGFPass0.WorldPosition')
    g.add_edge('GBufferRT.linearZ', 'SVGFPass0.LinearZ')
    g.add_edge('GBufferRT.pnFwidth', 'SVGFPass0.PositionNormalFwidth')
    g.add_edge('GBufferRT.normW', 'SVGFPass0.WorldNormal')
    g.add_edge('GBufferRT.mvec', 'SVGFPass0.MotionVec')
    ...
    g.add_edge('GBufferRT.emissive', 'MultipleIlluminationModulator.Emission')
...
try: 
    ...
    # frame capture
    m.frameCapture.outputDir = svgf_path
    ...
    with open(svgf_path + "/SVGF.csv", "w") as f:
